...
export const appRouter = router({
  ...
  staff: router({
    ...
    create: managerProcedure.input(z.object({ phone: z.string().min(3).max(32), password: z.string().min(6).max(120), name: z.string().min(2).max(160), title: z.string().max(120).optional(), department: z.string().max(120).optional(), baseSalary: z.number().int().min(0).default(0), role: z.enum(["manager","supervisor","employee"]).default("employee"), shiftStart: z.string().max(8).default("09:00"), shiftEnd: z.string().max(8).default("18:00") })).mutation(async ({ ctx, input }) => { const staff = await db.createStaffAccount({ ...input }); if (staff) { const actor = await enterprise.getCompanyForStaff(ctx.staffUser.id); if (actor) { const m = await enterprise.getMembership(staff.id); if (!m) { const dbx = await db.getDb(); if (dbx) { const { companyMembers } = await import("../drizzle/schema"); await dbx.insert(companyMembers).values({ companyId: actor.companyId, branchId: actor.branchId, staffAccountId: staff.id, role: input.role === "manager" ? "owner" : input.role === "supervisor" ? "supervisor" : "employee" }); } } await enterprise.createNotification(staff.id, "welcome", "مرحبًا بك في الشركة", "تم إنشاء حسابك ويمكنك الآن تسجيل الدخول."); } } return staffView(staff); }),
    update: managerProcedure.input(z.object({ id: z.number().int(), phone: z.string().min(3).max(32).optional(), password: z.string().min(6).max(120).optional(), name: z.string().min(2).max(160).optional(), title: z.string().max(120).optional(), department: z.string().max(120).optional(), baseSalary: z.number().int().min(0).optional(), role: z.enum(["manager","supervisor","employee"]).optional(), shiftStart: z.string().max(8).optional(), shiftEnd: z.string().max(8).optional(), active: z.boolean().optional() })).mutation(async ({ ctx, input }) => { const { id, ...changes } = input; await enterprise.assertStaffInCompany(ctx.staffUser.id, id); return staffView(await db.updateStaffAccount(id, changes)); }),
  }),
  company: router({
