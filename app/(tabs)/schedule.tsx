...
export default function ScheduleScreen() {
  ...
  return <ScreenContainer><FlatList
    ...
    ListHeaderComponent={<>
      <View style={styles.header}><View><Text style={styles.eyebrow}>{role === "manager" ? "توزيع ورديات الفريق" : "مواعيد عملك"}</Text><Text style={styles.title}>جدول الأسبوع</Text><Text style={styles.subtitle}>اعرف ميعادك قبل بداية كل يوم</Text></View><View style={styles.iconBubble}><IconSymbol name="calendar" size={24} color="#2563EB" /></View></View>
    ...
    renderItem={({ item }) => { const entry = role === "manager" ? findSchedule(visibleEntries, selectedEmployee, item.key) : visibleEntries.find((schedule) => schedule.scheduleDate === item.key); const isSelected = item.key === selectedDay; return <Pressable onPress={() => role === "manager" && setSelectedDay(item.key)} style={[styles.dayCard, isSelected && role === "manager" && styles.daySelected, item.key === week[0].key && styles.todayCard]}><View style={styles.dayTop}><View style={styles.datePill}><Text style={styles.datePillText}>{item.date}</Text></View><View><Text style={styles.dayName}>{item.label}</Text><Text style={styles.dayHint}>{item.key === week[0].key ? "وردية اليوم" : "جدول العمل"}</Text></View></View><View style={styles.dayDivider} />{entry?.shift ? <View style={styles.assignedRow}><View style={[styles.shiftMarker, { backgroundColor: entry.shift.kind === "weekly_off" ? "#D97706" : entry.shift.crossesMidnight ? "#7C3AED" : "#2563EB" }]} /><View style={styles.assignedCopy}><Text style={styles.assignedName}>{entry.shift.name}</Text><Text style={styles.assignedTime}>{shiftLabel(entry.shift)}</Text></View><IconSymbol name="checkmark" size={22} color="#059669" /></View> : <View style={styles.unassigned}><IconSymbol name="clock" size={22} color="#94A3B8" /><Text style={styles.unassignedText}>{role === "manager" ? "لم تحدد وردية — اختر اليوم ثم اضغط على وردية" : "لم يتم تحديد وردية لك بعد"}</Text></View>}</Pressable>}}
    ListFooterComponent={<View style={styles.footer}><IconSymbol name="info" size={19} color="#2563EB" /><Text style={styles.footerText}>الشيفت المسائي ينتهي الساعة 1:00 بعد منتصف الليل، ويُحسب حضوره على يوم بداية الشيفت.</Text></View>}
  /></ScreenContainer>;
}

const styles = StyleSheet.create({
  weekHero: { backgroundColor: "#0B1220", borderRadius: 23, padding: 18, flexDirection: "row-reverse", alignItems: "center", gap: 12 },
  weekHeroIcon: { width: 45, height: 45, borderRadius: 14, backgroundColor: "#2563EB", alignItems: "center", justifyContent: "center" },
  weekHeroCopy: { flex: 1 },
  weekHeroTitle: { color: "#FFFFFF", fontSize: 16, fontWeight: "800", textAlign: "right" },
  weekHeroText: { color: "#94A3B8", fontSize: 10, lineHeight: 16, marginTop: 3, textAlign: "right" },
  weekHeroCount: { alignItems: "center" },
  weekHeroNumber: { color: "#60A5FA", fontSize: 24, fontWeight: "900" },
  weekHeroLabel: { color: "#94A3B8", fontSize: 9 },
  content: { padding: 20, paddingBottom: 45, gap: 14 }, header: { flexDirection: "row-reverse", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }, eyebrow: { color: "#64748B", fontSize: 13, textAlign: "right" }, title: { color: "#0F172A", fontSize: 28, fontWeight: "800", marginTop: 5, textAlign: "right" }, subtitle: { color: "#64748B", fontSize: 12, marginTop: 5, textAlign: "right" }, iconBubble: { width: 50, height: 50, borderRadius: 17, backgroundColor: "#DBEAFE", alignItems: "center", justifyContent: "center" }, shiftLegend: { backgroundColor: "#1D4ED8", borderRadius: 20, padding: 16 }, legendTitle: { color: "#DBEAFE", fontSize: 12, fontWeight: "700", textAlign: "right", marginBottom: 12 }, legendRow: { flexDirection: "row-reverse", gap: 10 }, legendItem: { flex: 1, flexDirection: "row-reverse", alignItems: "center", gap: 7 }, legendDot: { width: 9, height: 9, borderRadius: 5 }, legendName: { color: "#FFFFFF", fontSize: 11, fontWeight: "800", textAlign: "right" }, legendTime: { color: "#DBEAFE", fontSize: 10, marginTop: 3, textAlign: "right" }, managerPanel: { backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#E2E8F0", borderRadius: 18, padding: 15 }, panelTitle: { color: "#0F172A", fontSize: 13, fontWeight: "800", textAlign: "right", marginBottom: 11 }, employeePicker: { flexDirection: "row-reverse", gap: 8 }, employeeChip: { borderWidth: 1, borderColor: "#CBD5E1", borderRadius: 12, paddingHorizontal: 12, paddingVertical: 9 }, employeeChipActive: { backgroundColor: "#DBEAFE", borderColor: "#2563EB" }, employeeChipText: { color: "#64748B", fontSize: 11 }, employeeChipTextActive: { color: "#2563EB", fontWeight: "800" }, selectedHint: { color: "#2563EB", fontSize: 11, textAlign: "right", marginTop: 11 }, assignPanel: { backgroundColor: "#F8FAFC", borderRadius: 18, padding: 14 }, shiftButtons: { flexDirection: "row-reverse", gap: 9 }, shiftButton: { flex: 1, backgroundColor: "#2563EB", borderRadius: 13, padding: 11, minHeight: 83 }, nightButton: { backgroundColor: "#6D28D9" }, offButton: { backgroundColor: "#B45309" }, disabled: { opacity: 0.55 }, shiftButtonName: { color: "#FFFFFF", fontSize: 11, fontWeight: "800", textAlign: "right" }, shiftButtonTime: { color: "#DBEAFE", fontSize: 10, marginTop: 5, textAlign: "right" }, assignText: { color: "#DBEAFE", fontSize: 9, marginTop: 8, textAlign: "right" }, dayCard: { backgroundColor: "#FFFFFF", borderRadius: 19, borderWidth: 1, borderColor: "#E2E8F0", padding: 15 }, todayCard: { borderColor: "#93C5FD" }, daySelected: { borderColor: "#2563EB", borderWidth: 2 }, dayTop: { flexDirection: "row-reverse", justifyContent: "space-between", alignItems: "center" }, dayName: { color: "#0F172A", fontSize: 15, fontWeight: "800", textAlign: "right" }, dayHint: { color: "#94A3B8", fontSize: 10, marginTop: 3, textAlign: "right" }, datePill: { backgroundColor: "#F1F5F9", borderRadius: 10, paddingHorizontal: 10, paddingVertical: 7 }, datePillText: { color: "#475569", fontSize: 11, fontWeight: "700" }, dayDivider: { height: 1, backgroundColor: "#F1F5F9", marginVertical: 13 }, assignedRow: { flexDirection: "row-reverse", alignItems: "center", gap: 10 }, shiftMarker: { width: 10, height: 40, borderRadius: 5 }, assignedCopy: { flex: 1 }, assignedName: { color: "#0F172A", fontSize: 13, fontWeight: "800", textAlign: "right" }, assignedTime: { color: "#64748B", fontSize: 12, marginTop: 4, textAlign: "right" }, unassigned: { flexDirection: "row-reverse", alignItems: "center", gap: 9 }, unassignedText: { color: "#94A3B8", fontSize: 11, flex: 1, textAlign: "right", lineHeight: 18 }, footer: { backgroundColor: "#EFF6FF", borderRadius: 15, padding: 13, flexDirection: "row-reverse", alignItems: "center", gap: 8, borderWidth: 1, borderColor: "#DBEAFE" }, footerText: { color: "#1E40AF", fontSize: 11, flex: 1, textAlign: "right", lineHeight: 18 }, emptyText: { color: "#94A3B8", fontSize: 11, textAlign: "right" } });
