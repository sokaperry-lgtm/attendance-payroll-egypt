...
export default function ScheduleScreen() {
  ...
  return <ScreenContainer><FlatList
    ...
    ListHeaderComponent={<>
      <View style={styles.header}><View><Text style={styles.eyebrow}>{role === "manager" ? "توزيع ورديات الفريق" : "مواعيد عملك"}</Text><Text style={styles.title}>جدول الأسبوع</Text><Text style={styles.subtitle}>اعرف ميعادك قبل بداية كل يوم</Text></View><View style={styles.iconBubble}><IconSymbol name="calendar" size={24} color="#668C7F" /></View></View>
    ...
    renderItem={({ item }) => { const entry = role === "manager" ? findSchedule(visibleEntries, selectedEmployee, item.key) : visibleEntries.find((schedule) => schedule.scheduleDate === item.key); const isSelected = item.key === selectedDay; return <Pressable onPress={() => role === "manager" && setSelectedDay(item.key)} style={[styles.dayCard, isSelected && role === "manager" && styles.daySelected, item.key === week[0].key && styles.todayCard]}><View style={styles.dayTop}><View style={styles.datePill}><Text style={styles.datePillText}>{item.date}</Text></View><View><Text style={styles.dayName}>{item.label}</Text><Text style={styles.dayHint}>{item.key === week[0].key ? "وردية اليوم" : "جدول العمل"}</Text></View></View><View style={styles.dayDivider} />{entry?.shift ? <View style={styles.assignedRow}><View style={[styles.shiftMarker, { backgroundColor: entry.shift.kind === "weekly_off" ? "#B77A32" : entry.shift.crossesMidnight ? "#A78BFA" : "#668C7F" }]} /><View style={styles.assignedCopy}><Text style={styles.assignedName}>{entry.shift.name}</Text><Text style={styles.assignedTime}>{shiftLabel(entry.shift)}</Text></View><IconSymbol name="checkmark" size={22} color="#2E7D68" /></View> : <View style={styles.unassigned}><IconSymbol name="clock" size={22} color="#8A918D" /><Text style={styles.unassignedText}>{role === "manager" ? "لم تحدد وردية — اختر اليوم ثم اضغط على وردية" : "لم يتم تحديد وردية لك بعد"}</Text></View>}</Pressable>}}
    ListFooterComponent={<View style={styles.footer}><IconSymbol name="info" size={19} color="#668C7F" /><Text style={styles.footerText}>الشيفت المسائي ينتهي الساعة 1:00 بعد منتصف الليل، ويُحسب حضوره على يوم بداية الشيفت.</Text></View>}
  /></ScreenContainer>;
}

const styles = StyleSheet.create({
  weekHero: { backgroundColor: "#FFFFFF", borderRadius: 23, padding: 18, flexDirection: "row-reverse", alignItems: "center", gap: 12 },
  weekHeroIcon: { width: 45, height: 45, borderRadius: 14, backgroundColor: "#668C7F", alignItems: "center", justifyContent: "center" },
  weekHeroCopy: { flex: 1 },
  weekHeroTitle: { color: "#FFFFFF", fontSize: 16, fontWeight: "800", textAlign: "right" },
  weekHeroText: { color: "#8A918D", fontSize: 10, lineHeight: 16, marginTop: 3, textAlign: "right" },
  weekHeroCount: { alignItems: "center" },
  weekHeroNumber: { color: "#60A5FA", fontSize: 24, fontWeight: "900" },
  weekHeroLabel: { color: "#8A918D", fontSize: 9 },
  content: { padding: 20, paddingBottom: 45, gap: 14 }, header: { flexDirection: "row-reverse", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }, eyebrow: { color: "#7B817E", fontSize: 13, textAlign: "right" }, title: { color: "#303735", fontSize: 28, fontWeight: "800", marginTop: 5, textAlign: "right" }, subtitle: { color: "#7B817E", fontSize: 12, marginTop: 5, textAlign: "right" }, iconBubble: { width: 50, height: 50, borderRadius: 17, backgroundColor: "#E8EEE9", alignItems: "center", justifyContent: "center" }, shiftLegend: { backgroundColor: "#2E5FD9", borderRadius: 20, padding: 16 }, legendTitle: { color: "#BFDBFE", fontSize: 12, fontWeight: "700", textAlign: "right", marginBottom: 12 }, legendRow: { flexDirection: "row-reverse", gap: 10 }, legendItem: { flex: 1, flexDirection: "row-reverse", alignItems: "center", gap: 7 }, legendDot: { width: 9, height: 9, borderRadius: 5 }, legendName: { color: "#FFFFFF", fontSize: 11, fontWeight: "800", textAlign: "right" }, legendTime: { color: "#BFDBFE", fontSize: 10, marginTop: 3, textAlign: "right" }, managerPanel: { backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#E7E2D9", borderRadius: 18, padding: 15 }, panelTitle: { color: "#303735", fontSize: 13, fontWeight: "800", textAlign: "right", marginBottom: 11 }, employeePicker: { flexDirection: "row-reverse", gap: 8 }, employeeChip: { borderWidth: 1, borderColor: "#8B96A8", borderRadius: 12, paddingHorizontal: 12, paddingVertical: 9 }, employeeChipActive: { backgroundColor: "#E8EEE9", borderColor: "#668C7F" }, employeeChipText: { color: "#7B817E", fontSize: 11 }, employeeChipTextActive: { color: "#668C7F", fontWeight: "800" }, selectedHint: { color: "#668C7F", fontSize: 11, textAlign: "right", marginTop: 11 }, assignPanel: { backgroundColor: "#10161F", borderRadius: 18, padding: 14 }, shiftButtons: { flexDirection: "row-reverse", gap: 9 }, shiftButton: { flex: 1, backgroundColor: "#668C7F", borderRadius: 13, padding: 11, minHeight: 83 }, nightButton: { backgroundColor: "#2A1B45" }, offButton: { backgroundColor: "#2B2410" }, disabled: { opacity: 0.55 }, shiftButtonName: { color: "#FFFFFF", fontSize: 11, fontWeight: "800", textAlign: "right" }, shiftButtonTime: { color: "#BFDBFE", fontSize: 10, marginTop: 5, textAlign: "right" }, assignText: { color: "#BFDBFE", fontSize: 9, marginTop: 8, textAlign: "right" }, dayCard: { backgroundColor: "#FFFFFF", borderRadius: 19, borderWidth: 1, borderColor: "#E7E2D9", padding: 15 }, todayCard: { borderColor: "#93C5FD" }, daySelected: { borderColor: "#668C7F", borderWidth: 2 }, dayTop: { flexDirection: "row-reverse", justifyContent: "space-between", alignItems: "center" }, dayName: { color: "#303735", fontSize: 15, fontWeight: "800", textAlign: "right" }, dayHint: { color: "#8A918D", fontSize: 10, marginTop: 3, textAlign: "right" }, datePill: { backgroundColor: "#F3F0EA", borderRadius: 10, paddingHorizontal: 10, paddingVertical: 7 }, datePillText: { color: "#8A918D", fontSize: 11, fontWeight: "700" }, dayDivider: { height: 1, backgroundColor: "#F3F0EA", marginVertical: 13 }, assignedRow: { flexDirection: "row-reverse", alignItems: "center", gap: 10 }, shiftMarker: { width: 10, height: 40, borderRadius: 5 }, assignedCopy: { flex: 1 }, assignedName: { color: "#303735", fontSize: 13, fontWeight: "800", textAlign: "right" }, assignedTime: { color: "#7B817E", fontSize: 12, marginTop: 4, textAlign: "right" }, unassigned: { flexDirection: "row-reverse", alignItems: "center", gap: 9 }, unassignedText: { color: "#8A918D", fontSize: 11, flex: 1, textAlign: "right", lineHeight: 18 }, footer: { backgroundColor: "#FFFFFF", borderRadius: 15, padding: 13, flexDirection: "row-reverse", alignItems: "center", gap: 8, borderWidth: 1, borderColor: "#D6E2DC" }, footerText: { color: "#93C5FD", fontSize: 11, flex: 1, textAlign: "right", lineHeight: 18 }, emptyText: { color: "#8A918D", fontSize: 11, textAlign: "right" } });
