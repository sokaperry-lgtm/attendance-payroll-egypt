...
export default function SettingsScreen() {
  ...
  const permissions = trpc.companyAdmin.permissions.useQuery(undefined, { enabled: role === "owner" });
  ...
  return (
    <ScreenContainer>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {role === "owner" && <Section title="مركز الصلاحيات المتقدم" subtitle="تحكم دقيق في كل وحدة لكل موظف بدون تغيير دوره الوظيفي.">
          <View style={styles.permissionMemberTabs}>{(permissions.data ?? []).filter((m:any) => m.membershipRole !== "owner").map((member:any) => <Pressable key={member.id} onPress={() => setPermissionMemberId(member.id)} style={[styles.permissionMember, (permissionMemberId ?? permissions.data?.find((m:any) => m.membershipRole !== "owner")?.id) === member.id && styles.permissionMemberActive]}><Text style={styles.permissionMemberName}>{member.name}</Text><Text style={styles.permissionMemberRole}>{roleLabel(member.membershipRole)}</Text></Pressable>)}</View>
...
const styles=StyleSheet.create({
  ...
  permissionMemberTabs:{flexDirection:"row-reverse",flexWrap:"wrap",gap:7,marginBottom:10},permissionMember:{borderWidth:1,borderColor:"#D9E0E8",borderRadius:12,paddingHorizontal:11,paddingVertical:8,backgroundColor:"#FBFCFE"},permissionMemberActive:{backgroundColor:"#EEF4FA",borderColor:"#9CB8D5"},permissionMemberName:{color:"#172033",fontSize:10,fontWeight:"900",textAlign:"right"},permissionMemberRole:{color:"#98A2B3",fontSize:8,textAlign:"right",marginTop:2},permissionGrid:{flexDirection:"row-reverse",flexWrap:"wrap",gap:7},permissionRow:{flexDirection:"row-reverse",alignItems:"center",gap:8,borderWidth:1,borderColor:"#EDF1F5",borderRadius:12,padding:9,minWidth:220,flex:1},permissionSwitch:{width:30,height:30,borderRadius:9,backgroundColor:"#F1F3F6",alignItems:"center",justifyContent:"center"},permissionSwitchOn:{backgroundColor:"#163A63"},permissionSwitchText:{color:"#667085",fontSize:12,fontWeight:"900"},permissionCopy:{flex:1},permissionLabel:{color:"#172033",fontSize:10,fontWeight:"900",textAlign:"right"},permissionKey:{color:"#98A2B3",fontSize:7,textAlign:"right",marginTop:2},memberList:{gap:7},memberRow:{borderWidth:1,borderColor:"#EDF1F5",borderRadius:14,padding:11,flexDirection:"row-reverse",alignItems:"center",gap:12},memberRowCompact:{flexDirection:"column",alignItems:"stretch"},memberInfo:{minWidth:190,flex:1},memberName:{color:"#172033",fontSize:12,fontWeight:"900",textAlign:"right"},memberMeta:{color:"#98A2B3",fontSize:9,textAlign:"right",marginTop:2},roleCaption:{color:"#163A63",fontSize:9,fontWeight:"800",textAlign:"right",marginTop:4},memberControls:{flex:2,gap:7},branchChips:{flexDirection:"row-reverse",flexWrap:"wrap",gap:6},roleChips:{flexDirection:"row-reverse",flexWrap:"wrap",gap:6},chip:{borderWidth:1,borderColor:"#D9E0E8",borderRadius:10,paddingHorizontal:9,paddingVertical:7,backgroundColor:"#FFFFFF"},chipActive:{backgroundColor:"#163A63",borderColor:"#163A63"},roleChipActive:{backgroundColor:"#31577F",borderColor:"#31577F"},chipText:{color:"#667085",fontSize:9,fontWeight:"800"},chipTextActive:{color:"#FFFFFF"},
